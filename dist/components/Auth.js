...
import { Input, Checkbox, Button, Space, Typography, Divider, IconKey, IconMail, IconLock, IconUser, Auth as SupabaseAuth, } from '@supabase/ui';
import md5 from 'md5';
// @ts-ignore
...
const getGravatarUrl = async (email) => {
    ...
    return (_c = (_b = (_a = data === null || data === void 0 ? void 0 : data.entry) === null || _a === void 0 ? void 0 : _a[0]) === null || _b === void 0 ? void 0 : _b.thumbnailUrl) !== null && _c !== void 0 ? _c : null;
};
const VIEWS = {
    SIGN_IN: 'sign_in',
    SIGN_UP: 'sign_up',
...
function Auth({ supabaseClient, className, style, socialLayout = 'vertical', socialColors = false, socialButtonSize = 'medium', providers, view = 'sign_in', redirectTo, onlyThirdPartyProviders = false, magicLink = false, }) {
    ...
    const Container = (props) => (<div className={containerClasses.join(' ')} style={style}>
      <Space size={8} direction={'vertical'}>
        <SocialAuth supabaseClient={supabaseClient} verticalSocialLayout={verticalSocialLayout} providers={providers} socialLayout={socialLayout} socialButtonSize={socialButtonSize} socialColors={socialColors} redirectTo={redirectTo} onlyThirdPartyProviders={onlyThirdPartyProviders} magicLink={magicLink}/>
    ...
    switch (authView) {
        ...
        case VIEWS.SIGN_UP:
            return (<Container>
          <EmailAuth id={authView === VIEWS.SIGN_UP ? 'auth-sign-up' : 'auth-sign-in'} supabaseClient={supabaseClient} authView={authView} setAuthView={setAuthView} defaultEmail={defaultEmail} defaultPassword={defaultPassword} setDefaultEmail={setDefaultEmail} setDefaultPassword={setDefaultPassword} redirectTo={redirectTo} magicLink={magicLink}/>
        </Container>);
        case VIEWS.FORGOTTEN_PASSWORD:
            return (<Container>
          <SupabaseAuth.ForgottenPassword supabaseClient={supabaseClient} setAuthView={setAuthView} redirectTo={redirectTo}/>
        </Container>);
        case VIEWS.MAGIC_LINK:
            return (<Container>
          <SupabaseAuth.MagicLink supabaseClient={supabaseClient} setAuthView={setAuthView} redirectTo={redirectTo}/>
        </Container>);
...
function SocialAuth(_a) {
    var { className, style, supabaseClient, children, socialLayout = 'vertical', socialColors = false, socialButtonSize, providers, verticalSocialLayout, redirectTo, onlyThirdPartyProviders, magicLink } = _a, props = __rest(_a, ["className", "style", "supabaseClient", "children", "socialLayout", "socialColors", "socialButtonSize", "providers", "verticalSocialLayout", "redirectTo", "onlyThirdPartyProviders", "magicLink"]);
    const buttonStyles = {
        github: {
            backgroundColor: '#333',
            color: 'white',
    ...
    return (<Space size={8} direction={'vertical'}>
      {providers && providers.length > 0 && (<React.Fragment>
          <Space size={4} direction={'vertical'}>
            ...
            <Space size={2} direction={socialLayout}>
              {providers.map((provider) => {
                ...
                return (<div key={provider} style={!verticalSocialLayout ? { flexGrow: 1 } : {}}>
                    <Button block type="default" shadow size={socialButtonSize} style={socialColors ? buttonStyles[provider] : {}} icon={<AuthIcon />} loading={loading} onClick={() => handleProviderSignIn(provider)} className="flex items-center">
    ...
    const handleSubmit = async (e) => {
        ...
        switch (authView) {
            ...
            case 'sign_up':
                ...
                const { user: signUpUser, session: signUpSession, error: signUpError, } = await supabaseClient.auth.signUp({
                    email,
                    password,
                }, {
                    redirectTo,
    ...
    return (<form id={id} onSubmit={handleSubmit}>
      <Space size={6} direction={'vertical'}>
        <Space size={3} direction={'vertical'}>
          <Input label="Email address" placeholder="Email..." autoComplete="on" defaultValue={email} icon={<IconMail size={21} stroke={'#666666'}/>} onChange={(e) => setEmail(e.target.value)}/>
          <Input label="Password" placeholder="Password..." type="password" defaultValue={password} autoComplete="new-password" icon={<IconKey size={21} stroke={'#666666'}/>} onChange={(e) => setPassword(e.target.value)}/>
          {authView === 'sign_up' && (<Input label="Display Name" placeholder="Display name..." name="display-name" type="text" defaultValue={displayName} autoComplete="on" icon={<IconUser size={21} stroke={'#666666'}/>} onChange={(e) => setDisplayName(e.target.value)}/>)}
        </Space>
        <Space direction="vertical" size={6}>
          <Space style={{ justifyContent: 'space-between' }}>
            <Checkbox label="Remember me" name="remember_me" id="remember_me" onChange={(value) => setRememberMe(value.target.checked)}/>
            {authView === VIEWS.SIGN_IN && (<Typography.Link href="#auth-forgot-password" onClick={(e) => {
                e.preventDefault();
          ...
          <Button htmlType="submit" type="primary" size="large" icon={<IconLock size={21}/>} loading={loading} block disabled={authView === VIEWS.SIGN_IN
            ? !email || !password
        ...
        <Space direction="vertical" style={{ textAlign: 'center' }}>
          {authView === VIEWS.SIGN_IN && magicLink && (<Typography.Link href="#auth-magic-link" onClick={(e) => {
                e.preventDefault();
                setAuthView(VIEWS.MAGIC_LINK);
            }}>
              Sign in with magic link
            </Typography.Link>)}
          {authView === VIEWS.SIGN_IN ? (<Typography.Link href="#auth-sign-up" onClick={(e) => {
                e.preventDefault();
                handleViewChange(VIEWS.SIGN_UP);
            }}>
              Don't have an account? Sign up
            </Typography.Link>) : (<Typography.Link href="#auth-sign-in" onClick={(e) => {
                e.preventDefault();
